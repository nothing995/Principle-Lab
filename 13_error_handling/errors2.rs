...
fn total_cost(item_quantity: &str) -> Result<i32, String> {
    let processing_fee = 1;
    let cost_per_item = 5;


    let qty = item_quantity.parse::<i32>().map_err(|_| "Error: Invalid input".to_string())?;
...
fn main() {
    let list = vec!["3", "10", "0", "abc", "-2", "5.5"];

    for &input in &list {
        match total_cost(input) {
            Ok(cost) => println!("Input: {} → Total cost: {}", input, cost),
